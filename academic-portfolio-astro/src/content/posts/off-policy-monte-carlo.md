...
date: "2020-08-01"
description: "How importance sampling lets us estimate value functions under a target policy using episodes collected by a different behavior policy."
tags:
...
$$E_{\pi}[X] = \sum x \pi(x) \text{ where } x \in X$$
...
where $\rho(x) = \frac{\pi(x)}{b(x)}$ and is called the importance sampling ratio.
...
$$E_{\pi}[X] = E_{b}[X_{\rho}(X)] \approx \frac{1}{n}\sum_{i=1}^{n}x_i \rho(x_i) \text{ where } x_i \sim b$$
...
In off-policy, we are trying to estimate value under the target policy $\pi(s)$ using returns following the behavior policy $b(s)$. We need to find the value of $\rho$ for each of the sampled returns. $\rho$ is the probability of trajectory under $\pi$ divided by the probability of trajectory under $b$.

$$V_{\pi}(s) = E_{b}[\rho G_t|S_t=s] \text{ ; } \rho = \frac{P(\text{trajectory under } \pi)}{P(\text{trajectory under } b)}$$

The probability of a trajectory under a policy can be given by:

$$P(\text{trajectory under policy}) = P(A_t, S_{t+1}, A_{t+1}, ..., S_T| S_t, A_{t:T})$$

where all actions are taken under the policy $b$.

As this is a Markov process, we can split the probability terms into:

$$P(A_t, S_{t+1}, ..., S_T|S_t, A_{t:T}) = \prod_{k=1}^{T} b(A_k|S_k)p(S_{k+1}|S_k, A_k)$$

$$\rho = \frac{P(\text{trajectory under } \pi)}{P(\text{trajectory under } b)} = \prod_{k=t}^{T-1}\frac{\pi(A_k|S_k)p(S_{k+1}|S_k, A_k)}{b(A_k|S_k)p(S_{k+1}|S_k, A_k)}$$

$$\rho = \prod_{k=t}^{T-1}\frac{\pi(A_k|S_k)}{b(A_k|S_k)}$$

## Incremental Implementation of Off-policy MC

Suppose we have a sequence of returns $G_1, G_2, ...G_{n-1}$ all starting in the same state and with corresponding weights $W_i$, then the weighted average of returns:

$$V_n = \frac{\sum_{k=1}^{n-1} W_kG_k}{\sum_{k=1}^{n-1} W_k}, \quad n \geq 2$$

and the cumulative sum of weights $C_{n+1} = C_n + W_{n+1}$ where $C_0 = 0$.

The update rule: $V_{n+1} = V_n + \frac{W_n}{C_n}[G_n-V_n]$, $n \geq 1$ and $V_1$ is arbitrary.

Here $W_n$ will be the importance sampling weight.

## Greedy $\pi$ policy

When the target policy $\pi$ is $\epsilon$ greedy, it is deterministic and $\pi(A_t/S_t) = 1$. So:

$$\rho = \prod_{k=t}^{T-1}\frac{1}{b(A_k|S_k)}$$

and $W$ can be updated with $W \leftarrow W \frac{1}{b(A_t|S_t)}$ for each time step in the trajectory.
